...
class Solution {
    public int myAtoi(String s) {
        ...
        for (int i=0;i<s.length();i++){
            char ch=s.charAt(i);
            if ((ch=='-' || ch=='+') && ( sb.indexOf("-")<0 && sb.indexOf("+")<0 ) ){
                sb.append(ch);
            }else if ( ch>='0' && ch<='9'){
                sb.append(ch);
            }else {
      ...
      //    result = Long.parseLong(sb.toString());
            ...
            if (bigInteger.compareTo(maxInt)>0) return Integer.MAX_VALUE;
            if (bigInteger.compareTo(minInt)<0) return Integer.MIN_VALUE;
